...
const PatientLogin = () => {
    ...
    const [patientLogin, { isLoading }] = useLoginPatientMutation()
    ...
    const handleFormSubmit = async (loginData) => {
        try {
            await patientLogin(loginData).unwrap()
            toast.success('Login Successful! Welcome to CareNest.')
        ...
        <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-indigo-50 via-white to-purple-50 py-12 px-4 sm:px-6 lg:px-8">
            ...
            <Card className="max-w-md w-full shadow-2xl border-0 bg-white/95 backdrop-blur-xs relative z-10">
                <CardHeader className="space-y-1 text-center">
                    ...
                    <CardTitle className="text-3xl font-bold tracking-tight text-slate-800">Patient Login</CardTitle>
                    <CardDescription className="text-base text-slate-500">
                        Access your health records and book doorstep clinical visits
                    </CardDescription>
                </CardHeader>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
                        ...
                        <Button
                            type="submit"
                            className="w-full bg-linear-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary font-bold h-11 shadow-md hover:shadow-lg transition-all duration-300"
                            disabled={isLoading}
                        >
                            {isLoading ? (
